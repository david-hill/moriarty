...
sudo dnf install dnf-plugin-system-upgrade NetworkManager-openvpn NetworkManager-openvpn-gnome -y

mkdir -p /etc/opt/chrome/policies/managed
cp etc/opt/chrome/policies/managed/* /etc/opt/chrome/policies/managed

if [ ! -e /etc/pki/tls/certs/2015-RH-IT-Root-CA.pem ]; then
  sudo scp root@192.168.1.37:/etc/pki/tls/certs/2015-RH-IT-Root-CA.pem /etc/pki/tls/certs/2015-RH-IT-Root-CA.pem
fi
...
if [ ! -e /proc/sys/net/ipv4/conf/tun0 ]; then
  ...
  sudo chmod 600 /etc/NetworkManager/system-connections/*.ovpn
...
if [ ! $(grep -qi rawhide /etc/redhat-release) ] && [ installl_rawhide == 1 ]; then
    ...
    get https://password.corp.redhat.com/RH-IT-Root-CA.crt /etc/pki/ca-trust/source/anchors/RH-IT-Root-CA.crt
    get https://certs.corp.redhat.com/certs/2022-IT-Root-CA.pem /etc/pki/ca-trust/source/anchors/2022-IT-Root-CA.pem
    get https://certs.corp.redhat.com/certs/Current-IT-Root-CAs.pem /etc/pki/ca-trust/source/anchors/Current-IT-Root-CAs.pem
